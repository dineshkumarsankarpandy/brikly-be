...
async def get_all_projects(db: Session = Depends(get_db)):
    try:
        project = db.query(Sitemap.id, Sitemap.project_name, Sitemap.created_at, Sitemap.updated_at).all()
        if not project:
            return {
                "status": False,
        ...
        return {
            "data": [
                {"id": s.id, "project_name": s.project_name, "updated_at": s.updated_at,"created_at": s.created_at} for s in project
            ]
        }
